...
//			}
			...
			TCComponentContextList list = query.getExecuteResultsList(keys, values);
			if (list != null) {
	...
	public static void createBom(TCComponentItemRevision parentRev,
		...
		if (parentRev == null)
			return;
		String[] setProps = { "bl_quantity", "bl_sequence_no" }; // {
																	// JFomUtil.JF3_BOM_QUERY_NO,JFomUtil.JF3_BOM_COUNT_NO
																	// };
		TCTypeService service = session.getTypeService();
		TCComponentBOMWindowType winType = (TCComponentBOMWindowType) service
				.getTypeComponent("BOMWindow");
